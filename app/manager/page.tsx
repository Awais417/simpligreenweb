...
export default function ManagerJobsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setJobs(await api.manager.listJobs());
      } catch (err) {
    ...
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <EmptyState title="No jobs assigned" description="Jobs assigned to you by an admin will appear here." />
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-left text-xs text-gray-400 uppercase tracking-wide">
                  ...
                  <th className="px-6 py-3 font-medium">QA Reviewer</th>
                ...
                {jobs.map((job) => (
                  <tr key={job.id} className="hover:bg-gray-50/60">
                    <td className="px-6 py-3.5">
                      <Link href={`/manager/jobs/${job.id}`} className="font-medium text-gray-900 hover:text-brand">
                        {job.title}
                      </Link>
