...
export default function QaJobsPage() {
  usePageTitle('Jobs');
    ...
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <EmptyState title="No jobs to review" description="Jobs submitted for QA review will appear here." />
        ) : (
                ...
                {jobs.map((job) => (
                  <tr key={job.id} className="hover:bg-gray-50/60">
                    <td className="px-6 py-3.5">
                      <Link href={`/qa/jobs/${job.id}`} className="font-medium text-gray-900 hover:text-brand">
                        {job.title}
                      </Link>
                    ...
                    <td className="px-6 py-3.5 text-gray-500">{formatDate(job.updatedAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
